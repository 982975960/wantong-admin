wantong.cms.booklist_resource = (function () {
  var _root = null,
      ...
      _initModuleBookList = function () {
        ...
        //书本资源编辑
        _root.editResourcesBookList = $("#editResourcesBookList");
        //书本资源待审核模块
        _root.inExamineResourcesBookList = $("#inExamineResourcesBookList");
        //书本资源已发布模块
        _root.publishedResourcesBookList = $("#publishedResourcesBookList");
        // 书本资源禁用中模块
        _root.inForbiddenBookList = $("#inForbiddenBookList");
        //搜索全部书本资源
        _root.searchAllBookList = $("#searchAllBookList");
        //图片更新记录列表
        _root.pictureUpdateRecordList = $("#pictureUpdateRecordList");
      ...
      _initTab = function (activeIndex) {
        _root.find("#listTab").find("li").click(function () {
          _root.find("#importRepo").css("display", "inline");
          _clearSearch(true);
          ...
          if (index == 0){
            if (_bookIdAuth == 1){
              ...
              _root.find("#bookId").parents("Li").css("display", "none");
            }
          }else {
            _root.find("#bookId").parents("Li").css("display", "inline");
          }
          ...
          if (index == 0) {
            ...
            _root.find("#importRepo").css("display", "none");
            _clearSearch(true)
          } else if (index == 1) {
            _root.editResourcesBookList.show();
            _currentTab = _root.editResourcesBookList;
            _bookState = 0;

            _refreshList();
          } else if (index == 2) {
            _root.inExamineResourcesBookList.show();
            _currentTab = _root.inExamineResourcesBookList;
            _bookState = 7;

            _refreshList();
          } else if (index == 3) {
            _root.publishedResourcesBookList.show();
            _currentTab = _root.publishedResourcesBookList;
            _bookState = 3;

            _refreshList();
          } else if (index == 10) {
            _root.inForbiddenBookList.show();
            _currentTab = _root.inForbiddenBookList;

            _root.find("#importRepo").css("display", "none");
            _refreshList();
          } else if (index == 100) {
            _root.searchAllBookList.show();
            _currentTab = _root.searchAllBookList;
            _hideTimeSelectAndShow(true);
            _root.find("#importRepo").css("display", "none");
          }else if(index == -1){
            //当选择 图片更新记录模块
            _clearSearch(true);
            _root.pictureUpdateRecordList.show();
            $("#searchTerms").hide();
            _currentTab = _root.pictureUpdateRecordList;
            _refreshPictureUpdateList(1);
          }
      ...
      _hideAllTab = function () {
        ...
        _root.editResourcesBookList.hide();
        _root.inExamineResourcesBookList.hide();
        _root.publishedResourcesBookList.hide();
        _root.inForbiddenBookList.hide();
        _root.searchAllBookList.hide();
        _root.pictureUpdateRecordList.hide();
        $("#searchTerms").show();
        _refHint.hide();
      },
      //显示或者隐藏元素
      ...
      _refreshPictureUpdateList = function(page){
        parameter.modelId =  _root.find("#modelId").val();
        parameter.module = -1;
        if(page == undefined){
          page = 1;
        }
      ...
      _refreshList = function (page, isSelectPool) {
        ...
        //搜索模块切换资源库是不需要搜索
        if (_currentTab == _root.searchAllBookList && isSelectPool) {
          return;
        }
        _setUpParameters();
        ...
        if (_currentTab == _root.referenceBookList) {
          ...
          parameter.module = 0;
        } else if (_currentTab == _root.editResourcesBookList) {
          //  资源制作
          parameter.status = REPO_MAKE_STATE;
          status = REPO_MAKE_STATE
          parameter.module = 1;
        } else if (_currentTab == _root.inExamineResourcesBookList) {
          //  待审核
          parameter.status = REPO_CHECK_STATE;
          status = REPO_CHECK_STATE;
          parameter.module = 3;
        } else if (_currentTab == _root.publishedResourcesBookList) {
          //  已审核
          parameter.status = REPO_PUBLISH_STATE
          status = REPO_PUBLISH_STATE;
          parameter.module = 4;
        } else if (_currentTab == _root.inForbiddenBookList) {
          //  禁用
          parameter.forbidden = 1;
          parameter.module = 2;
          parameter.status = "";
      ...
      _initButtons = function () {
        ...
        _root.delegate("a[forbidden]", "click", function (event) {
          event.stopPropagation();

          var bookId = $(event.currentTarget).attr("bookId");
          var forbidden = $(event.currentTarget).attr("forbidden");
          _forbiddenBook(bookId, forbidden, $(this));
        });

        _root.delegate("#showQrcodeLi", "click", function (event) {
          event.stopPropagation();
          var bookId = $(event.currentTarget).attr("bookId");
          _showQrcode(bookId);
        });
        ...
        //图像修改记录打开书本的点击事件
        _root.delegate("#updateBook","click",function (event) {
          event.stopPropagation();
          parameter._searchContent.bookId = $(event.currentTarget).attr("bookId");
        ...
        //查看修改记录
        _root.delegate("#lookOver","click",function (event) {
            event.stopPropagation();
            $(this).blur();
            _openDig(parseInt($(this).attr("bookid")));
        });
        ...
        _root.delegate('#packBookBtn', 'click', function (event) {
          event.stopPropagation();

          $(this).blur();
          var id = $(this).attr('bookid');
          layer.confirm('是否重新打包该书本资源?',
              {
                icon: 3,
                title: '确认重新打包'
              },
        ...
        _root.delegate("#refBookBtn", "click", function (event) {
          ...
          var repoId = _root.find("#modelId").val();
...
//loading层
      ...
      _initUploader = function () {
        ...
        _uploader = WebUploader.create({
          ...
          server: _conf.UPLOAD_BIG_FILE,
          pick: {
            id: "#importRepo"
          },
          duplicate: true, //将dupliacate设置为true,  允许文件多次上传
          auto: true,
          resize: false,
          chunked: true,//开始分片上传
        ...
        _uploader.on("fileQueued", function () {
          _indexk12 = layer.msg('资源上传中...', {icon: 16,shade: [0.5, '#f5f5f5'],scrollbar: false,offset: 'auto', time: 1000000});
        });
        // 所有文件上传成功后调用
        ...
        _uploader.on("uploadSuccess", function (file, res) {
          ...
          if (res.code == 0){
            ...
            $.post(_conf.UPLOAD_URL,{
              ...
              if (response.code == 0){
                layer.close(_indexk12);
                var reData = response.data;
                if (reData.flag == 1){
                  var indexConfirm = layer.confirm('当前资源库已存在相同书本，继续导入则会覆盖之前的资源，确定继续导入吗?',
                    {btn: ['确认', '取消'], title: "提示"},
                    function () {
                      $(".layui-layer-btn").empty().html("<button class='frame-Button-b'>导入音频中</button>");
                      $.post(_conf.UPLOAD_CONFIRM,{
                        tempFilePath: reData.tempFilePath,
                        decryptPath: reData.decryptPath,
                        tempFolder: reData.tempFolder,
                        bookId: reData.bookId,
                        repoId: reData.repoId,
                        bookState: _bookState
                      },function (data) {
      ...
      _forbiddenBook = function (bookId, forbidden, e) {

        $.post(_conf.FORBIDDEN_BOOK_URL, {
          bookId: bookId,
          forbidden: forbidden
        }, function (data) {
          if (data.code == 0) {
            if (forbidden == 0) {
              e.attr("forbidden", 1).text("启用");
            } else if (forbidden == 1) {
              e.attr("forbidden", 0).text("禁用");
            }
            setTimeout(function () {
              layer.msg(forbidden == 0 ? "禁用成功":"启用成功");
              _refreshCurrentPage();
            },1000);
          } else {
            layer.msg("操作失败");
      ...
      _showQrcode = function(bookId){
        $.post(_conf.SHOW_QRCODE_URL, {
          bookId: bookId
        }, function (data) {
          if (data.code == 0){
            layer.open({
      ...
      _setUpParameters = function () {
        ...
        let lableIdString = _root.find("input[name='ids']").val();
        parameter._searchContent.labelIds = lableIdString == ""?null:lableIdString.split(",");
        ...
        parameter.stateText  = _root.find("#stateSelector").find("input[name='text']").val();
        parameter.stateArray = parameter.stateText ==""?null:parameter.stateText.split(",");
        parameter.status = "";
      },
  ...
  return {
    init: function (activeIndex, bookIdAuth) {
      _init(activeIndex, bookIdAuth);
    },
