...
var DELETE_REPEAT_ROUND = 10;
//仅点击切换区域
var ONLY_CLICK_TOGGLE = false;
//完成绘制回调
var finish_draw_recall = null;
var click_canvas_recall = null;
var click_selected_recall = null;
var close_tips_index = null;
var close_tip = false;
// 1280 720
// 12   39
var manager_width = $('#picManager').width();
var pannel_width = manager_width - 25;//'1012';//'756';
var pannel_height = manager_width - 25;//'537'//'393';


function isInRepeatPoint(lx, ly, cx, cy) {
  return lx >= cx - DELETE_REPEAT_ROUND && lx <= cx + DELETE_REPEAT_ROUND
  && ly >= cy - DELETE_REPEAT_ROUND && ly <= cy + DELETE_REPEAT_ROUND;
}
...
var VIA_REGION_EDGE_TOL           = 5;   // pixel 边界操作判断值
var VIA_REGION_CONTROL_POINT_SIZE = 2;
var VIA_REGION_POINT_RADIUS       = 7; // 点显示大小
var VIA_POLYGON_VERTEX_MATCH_TOL  = 5;
var VIA_REGION_MIN_DIM            = 3;
var VIA_MOUSE_CLICK_TOL           = 2;//2
...
var VIA_CANVAS_ZOOM_LEVELS = [0.25, 0.5, 0.8, 1.0, 1.5, 2.0, 2.5, 3.0, 4, 5, 6, 7, 8, 9, 10];
...
var VIA_THEME_SEL_REGION_FILL_COLOR = "#46481d";
var VIA_THEME_SEL_REGION_FILL_BOUNDARY_COLOR = "yellow";
var VIA_THEME_UNSELECT_COLOR = "#837412"; //未选中的颜色
var VIA_THEME_UNSELECT_FILL_COLOR = "#808080";
var VIA_THEME_SEL_REGION_OPACITY    = 0.6;
...
var _via_img_metadata = {};   // 图层数据集 data structure to store loaded images metadata
...
var _via_canvas_scale   = 1;// current scale of canvas image

var _via_image_id       = ''; // id={filename+length} of current image
var _via_image_index    = -1; // index  当前imageIndex
...
var _via_image_id_list                 = []; // imageId列表 array of all image id (in order they were added by user)
var _via_image_filename_list           = []; // image文件名列表 array of all image filename
...
var _via_img_fn_list_img_index_list    = []; // imageIndex 界面可视列表 image index list of images show in img_fn_list
var _via_img_fn_list_html              = []; // image 界面可视列表HTMLhtml representation of image filename list
...
function _via_init() {
  ...
  // e.g. demo modules
  if (typeof wantong.cms.pageFingerAdd.via_load_submodules === 'function') {
    console.log('Loading VIA submodule');
    setTimeout( async function() {
      await wantong.cms.pageFingerAdd.via_load_submodules();
    }, 100);
  }

  ui_top_panel.style.display = 'none';
...
function _via_init_mouse_handlers() {
  ...
  document.addEventListener('mouseup', _via_reg_document_mouseup_handler, false);
}
...
function import_annotations_from_json_obj(data) {
  return new Promise( function(ok_callback, err_callback) {
    var d;
...
function _via_load_canvas_regions() {
  ...
  if (!_via_img_metadata[_via_image_id]) {
    return;
  }
...
function _via_reg_canvas_mousedown_handler(e) {
  ...
  var res = is_inside_this_region(_via_click_x0, _via_click_y0, _via_user_sel_region_id);
  console.log('inside this', res);

  var region_id = res ? _via_user_sel_region_id : is_inside_region(_via_click_x0, _via_click_y0);
  //优先选中
  var select_edge = is_in_region_corner(_via_current_x, _via_current_y, _via_user_sel_region_id);
  if (res) {
    _via_region_edge = select_edge;
  } else  {
    if (select_edge[0] != -1) {
      _via_region_edge = select_edge;
    } else {
  ...
  if ( _via_is_region_selected) {
    //console.log("当前勾选状态", region_id, _via_region_selected_flag[region_id], _via_region_edge);
    // check if user clicked on the region boundary
    console.log('select click!!!');
    if (click_selected_recall) {
      click_selected_recall();
    }
    console.log('_via_is_region_selected', _via_is_region_selected);
    if ( _via_region_edge[1] > 0 ) {
      console.log('_via_region_edge', _via_region_edge);
      if ( !_via_is_user_resizing_region ) {
        if (_via_region_edge[0] !== _via_user_sel_region_id) {
          // console.log('点击非当前选中region', _via_region_edge[0]);
        } else {
          if ( _via_region_edge[0] !== _via_user_sel_region_id ) {
...
function _via_reg_document_mouseup_handler(e){
  if ( _via_is_user_moving_region) {
    //canvas外释放鼠标直接还原位置
...
function _via_reg_canvas_mouseup_handler(e) {
  ...
  if ( _via_is_user_moving_region ) {
    ...
    if (Math.abs(move_x) > VIA_MOUSE_CLICK_TOL ||
      ...
      console.log('move all selected regions');
      ...
      console.log("nested", nested_region_id, _via_user_sel_region_id);
      if (nested_region_id >= 0 &&
          nested_region_id !== _via_user_sel_region_id) {
        if (!ONLY_CLICK_TOGGLE) {
          _via_user_sel_region_id = nested_region_id;
          _via_is_region_selected = true;
          _via_is_user_moving_region = false;
          click_canvas_recall(nested_region_id);
  ...
  if ( click_dx < VIA_MOUSE_CLICK_TOL ||
    ...
    if ( _via_is_user_drawing_polygon ) {
      console.log('_via_is_user_drawing_polygon', _via_is_user_drawing_polygon);
      var canvas_x0 = Math.round(_via_click_x1);
      var canvas_y0 = Math.round(_via_click_y1);
      ...
      //点击上个点删除
      //判断是否在圈内
      if (isInRepeatPoint(last_x0, last_y0, canvas_x0, canvas_y0)) {
        if (n == 1) {
          console.log('exit');
          //退出绘画
          _via_canvas_regions.splice(_via_current_polygon_region_id, 1);
          ...
          //删除上个点
          console.log('delete last vertex');
          _via_polyshape_delete_last_vertex();
        }
      } else {
        //点击第一个点闭合
        if (n > 2) {
          var first_x0 = _via_canvas_regions[_via_current_polygon_region_id].shape_attributes['all_points_x'][0];
          var first_y0 = _via_canvas_regions[_via_current_polygon_region_id].shape_attributes['all_points_y'][0];
          if (isInRepeatPoint (first_x0, first_y0, canvas_x0, canvas_y0)) {
      ...
      //判定点击是否在矩形内
      if ( region_id >= 0 ) {
        if (!ONLY_CLICK_TOGGLE || !_via_is_region_selected) {
          console.log('check only toggle');
          // yongchuan
          console.log("click up in " + region_id);
          selected_region_id = region_id;
  ...
  if ( _via_is_user_drawing_region ) {
    ...
    if ( region_dx > VIA_REGION_MIN_DIM && region_dy > VIA_REGION_MIN_DIM ) { // avoid regions with 0 dim
      ...
      if ( new_region_added ) {
        ...
        annotation_editor_show();
        if (finish_draw_recall) {
          finish_draw_recall(VIA_REGION_SHAPE.RECT, new_region_id);
        }
      }
      _via_redraw_reg_canvas();
      _via_reg_canvas.focus();
...
function _select_region(region_id) {
  // first click selects region
  _via_user_sel_region_id     = region_id;
  _via_is_region_selected     = true;
  _via_is_user_moving_region  = false;
...
function _via_close_poly_tips(clientX , clientY, e) {
  if ( !_via_current_image_loaded ) {
    return;
  }
  var canvas_x0 = Math.round(clientX);
  var canvas_y0 = Math.round(clientY);
  if ( _via_is_user_drawing_polygon && _via_canvas_regions[_via_current_polygon_region_id].shape_attributes['all_points_x'].length > 2) {
    var n = _via_canvas_regions[_via_current_polygon_region_id].shape_attributes['all_points_x'].length;
    var first_x0 = _via_canvas_regions[_via_current_polygon_region_id].shape_attributes['all_points_x'][0];
    var first_y0 = _via_canvas_regions[_via_current_polygon_region_id].shape_attributes['all_points_y'][0];
    var check = isInRepeatPoint (first_x0, first_y0, canvas_x0, canvas_y0);
    //console.log('check pos', first_x0, first_y0, canvas_x0, canvas_y0, n, check);
    if (check) {
      $('#close_tip').css('top', first_y0).css('left', first_x0);
      if (close_tip) {
        return;
      }
      close_tip = true;
      close_tips_index = layer.tips('点击该点将会自动连接为多边形', '#close_tip',{
        time: 0
      });
    } else {
      if (close_tips_index) {
        close_tip = false;
        layer.close(close_tips_index);
      }
    }
  }
}
...
function _via_reg_canvas_mousemove_handler(e) {
  ...
  // console.log('_via_reg_canvas_mousemove_handler');
  if ( _via_is_region_selected ) {
    if ( !_via_is_user_resizing_region ) {
      ...
      var select_edge = is_in_region_corner(_via_current_x, _via_current_y, _via_user_sel_region_id)
      if (select_edge[0] != -1) {
        // console.log('resize', select_edge[0])
        _via_region_edge = select_edge;
      // if ( _via_region_edge[0] === _via_user_sel_region_id ) {
        switch(_via_region_edge[1]) {
          // rect
  ...
  if ( _via_is_user_drawing_polygon ) {
    ...
    if ( npts > 0 ) {
      ...
      _via_close_poly_tips(_via_current_x, _via_current_y, e);
    }
...
function draw_all_regions() {
  //yongchuan
  var region_ids_arranged = [];
  for (var i = 0; i <= _via_canvas_regions.length - 1; i++) {
    region_ids_arranged.push(i);
  }
  if (selected_region_id >= 0) {
    region_ids_arranged.push(region_ids_arranged.splice(selected_region_id, 1)[0]);
    // console.log("selected region id " + selected_region_id);
    // console.log(region_ids_arranged);
  }

  var aid = _via_settings.ui.image.region_color;
  var attr, is_selected, aid, avalue;
  for (var j=0; j < _via_canvas_regions.length; ++j) {
    // yongchuan
    var i = region_ids_arranged[j];
    // console.log("i is " + i);
    ...
    case VIA_REGION_SHAPE.POLYGON:
      _via_draw_polygon_region(attr['all_points_x'],
                               attr['all_points_y'],
                               is_selected,
                               attr['name'], i ==  _via_canvas_regions.length - 1);
      break;
...
function _via_draw_rect_region(x, y, w, h, is_selected) {
  if (is_selected) {
    ...
    // draw a fill line
    _via_reg_ctx.strokeStyle = VIA_THEME_UNSELECT_COLOR;
    _via_reg_ctx.lineWidth   = VIA_THEME_REGION_BOUNDARY_WIDTH/2;
    _via_draw_rect(x, y, w, h);
    _via_reg_ctx.stroke();
    //fill
    _via_reg_ctx.fillStyle   = VIA_THEME_UNSELECT_FILL_COLOR;
    _via_reg_ctx.globalAlpha = VIA_THEME_SEL_REGION_OPACITY;
    _via_reg_ctx.fill();
    _via_reg_ctx.globalAlpha = 1.0;
 ...
 * @private
 */
function _via_draw_polygon_region(all_points_x, all_points_y, is_selected, shape, last_one) {
  if ( is_selected ) {
    _via_reg_ctx.strokeStyle = VIA_THEME_SEL_REGION_FILL_BOUNDARY_COLOR;
    _via_reg_ctx.lineWidth   = VIA_THEME_REGION_BOUNDARY_WIDTH/2;
    _via_reg_ctx.beginPath();
    ...
    if (_via_is_user_drawing_polygon && last_one) {
      _via_reg_ctx.strokeStyle = VIA_THEME_SEL_REGION_FILL_BOUNDARY_COLOR;
    } else {
      _via_reg_ctx.strokeStyle = VIA_THEME_UNSELECT_COLOR;
    }
...
//判断坐标是否在region的角上
function is_in_region_corner(px, py, region_id) {
...
function _via_reg_canvas_keydown_handler(e) {
  ...
  if (_via_current_image_loaded) {
    ...
    //删除选中的多边形
    if ( e.key === 'Delete' ) {
        $('.active-warpper .delete-btn').click();
    }
...
function _via_polyshape_finish_drawing() {
  if ( _via_is_user_drawing_polygon ) {
    ...
    _via_reg_canvas.focus();
    if (finish_draw_recall) {
      finish_draw_recall(VIA_REGION_SHAPE.POLYGON, new_region_id);
    }
  }
  return;
}
...
function delete_region(region_id) {
  ...
  del_sel_regions(region_id);
}

function delete_all_region() {
  if ( _via_display_area_content_name === VIA_DISPLAY_AREA_CONTENT_NAME.IMAGE_GRID ) {
    return;
  }
...
function img_fn_list_clear_css_classname(classname) {
  if (document.getElementById('img_fn_list').childNodes.length > 0) {
    var cn = document.getElementById('img_fn_list').childNodes[0].childNodes;
    var i;
    var n = cn.length;
    for ( i = 0; i < n; ++i ) {
...
/**
 *  遍历_via_attributes (用于存储全局默认值)， 设置图片属性为默认值
 * @param image_id
 */
...
function _via_show_img_from_buffer(img_index) {
  return new Promise( function(ok_callback, err_callback) {
    ...
    pannel_width = manager_width - 25;
    var image_panel_width = pannel_width;
    console.log('panel ', pannel_width, image_panel_width);
    // if ( leftsidebar.style.display === 'none' ) {
    //   image_panel_width = de.clientWidth;
    // }
    //console.log('panel ', pannel_width, image_panel_width);
    //var image_panel_height = de.clientHeight - 2*ui_top_panel.offsetHeight;
    var image_panel_height = pannel_height;
