  ...
  var RichEditor = function (container, params) {
          ...
          click: function () {
            ...
            function btnClick() {
              ...
              addEvent(confirm, 'click', function () {
                ...
                if (link.value.trim() != '') {
                  var a = '<a href="' + link.value + '" target="_blank">'
                      + link.value + '</a>';
    ...
    var fireEvent = function (element, type, bubbles, cancelable) {
      if (document.createEvent) {
        ...
        event.initEvent(type, bubbles !== undefined ? bubbles : true,
    ...
    r.drawEmotion = function () {
      var list_smilies = ['smile', 'smiley', 'yum', 'relieved', 'blush',
        'anguished', 'worried', 'sweat',
        'unamused', 'sweat_smile', 'sunglasses', 'wink', 'relaxed', 'scream',
        'pensive',
        'persevere', 'mask', 'no_mouth', 'kissing_closed_eyes', 'kissing_heart',
        'hushed',
        'heart_eyes', 'grin', 'frowning', 'flushed', 'fearful', 'dizzy_face',
        'disappointed_relieved',
        'cry', 'confounded', 'cold_sweat', 'angry', 'anguished', 'broken_heart',
        'beetle', 'good', 'no', 'beer',
        'beers', 'birthday', 'bow', 'bomb', 'coffee', 'cocktail', 'gun',
        'metal', 'moon'
      ];
      var html = '';
      list_smilies.forEach(function (v) {
        html += '<img src="images/emotion/' + v
            + '.png" class="emotion" width="20" height="20" alt="" />';
      });
    ...
    r.getStyle = function (dom, attr) {
      var value = dom.currentStyle ? dom.currentStyle[attr] : getComputedStyle(
          dom, false)[attr];
      return parseFloat(value);
    };
    r.openModal = function (html, fn) {
      console.log("22333");
      if ($("#editor .editor-modal").length > 0) {
        $("#editor .editor-modal").remove();
      }
      r.modal = document.createElement('div');
      r.modal.className = 'editor-modal';
      r.modal.innerHTML = html;
      r.parent.appendChild(r.modal);
      var left = this.offsetLeft + (r.getStyle(this, 'width') - r.getStyle(
          r.modal, 'width')) / 2;
      left < 0 ? left = 3 : '';
      ...
      this.addColorBoard = function () {
        ...
        for (var row = 1; row < 15; ++row) // should be '16' - but last line looks so dark
        {
          ...
          for (var col = 0; col < 25; ++col) // last column is grayscale
          {
            ...
            td.style.cursor = 'url(/supplier/email.do),crosshair';
