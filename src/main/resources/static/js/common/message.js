...
;(function(global,$){
	 ...
	 */
	 var cssMap={
		'.toast-message':'{position:fixed;left:56%;top:10%;transform:translateX(-50%);border:1px solid #EBEEF5;transition:top .3s ease-out;opacity .3s ease-out;padding:10px 0px 10px 10px;display:flex;align-items:center;z-index:2000; border-radius:10px;-moz-box-shadow: 2px 2px 5px #e8e8e8;box-shadow: 2px 2px 5px #e8e8e8;}',
		'.toast-icon':'',
		'.toast-message__closeBtn':'{position:absolute;top:45%;right:15px;transform:translateY(-35%);cursor:pointer;color:#C0C4CC;font-size:16px;width:26px;height:26px;line-height:26px;text-align:center;}',
		...
		'.toast-message--success':'{border-color:#cccccc;background-color:#ffffff;}',
	...
	$.message=function(options){
		if(options){
			...
			setTimeout(function(){
				...
				if(options.center){
					ele.style.top="45%";
					ele.style.transform="translate(-45%)";
				}
			},isIE()?10:5);
