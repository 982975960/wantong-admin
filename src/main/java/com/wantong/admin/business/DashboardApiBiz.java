...
public class DashboardApiBiz {
     ...
     * @return ApiResponse
     */
    public Object setup(AdminSession adminSession, Integer type, String minDate, String maxDate) {
        ...
        if (partnerId == 1) {
            //玩瞳
            optionalPartnerList = supplierRelatedService.listPartnersByStatus(1)
                    .stream()
                    .map(e -> new PartnerVO(e.getId(), e.getName()))
                    .collect(Collectors.toList());
            List<Long> idList = statisticsService.listPartnerIdOrder(type);
            //根据idList进行排序
            optionalPartnerList.sort(((o1, o2) -> {
                int io1 = idList.indexOf(o1.getPartnerId());
                int io2 = idList.indexOf(o2.getPartnerId());
                if (io1 != -1) {
                    io1 = optionalPartnerList.size() - io1;
                }

                if (io2 != -1) {
                    io2 = optionalPartnerList.size() - io2;
                }
        ...
        DatePeriod dateBound = null;
        if (!minDate.equals("") && !maxDate.equals("")) {
            dateBound = DatePeriod.parse(minDate, maxDate);
        } else {
            dateBound = statisticsService.statisticsDateBound(null);
        }
    ...
    private byte[] getExcel(String tableName, List<StatisticsPO> data) throws Exception {
        ...
        String filepath =
                storageConfig.getBookImportReport() + "/dashboard/" + UUID.randomUUID().toString() + Constants.DOT_XLSX;
